...
export function SimulationPanel({ circuit }: SimulationPanelProps) {
  ...
  const chartData = useMemo(() => {
    if (!stats) return [];
    return [
      { status: "powered", count: stats.powered, fill: "hsl(142 76% 36%)" },
      {
        status: "notPowered",
        ...
        <div className="flex flex-col gap-4">
          <div className="rounded-xl border bg-card p-4">
            <div className="mb-3 flex items-center justify-between">
              <div>
                <h3 className="font-semibold">Schéma interactif</h3>
                <p className="text-sm text-muted-foreground">
                  Cliquez sur les disjoncteurs pour les activer/désactiver
                </p>
              </div>
              {result && stats && (
                <Badge
                  className={cn(
                    ...
                    stats.percentage === 100
                      ? "bg-powered/10 text-powered"
                      : stats.percentage === 0
                        ? "bg-unpowered/10 text-unpowered"
                        : "bg-warning/10 text-warning"
                  )}
                >
                  {stats.powered}/{stats.total} alimentés
                </Badge>
              )}
            </div>
            <CircuitDiagram
